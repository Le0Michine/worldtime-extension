...
export function createTimeZoneInfo(
  timeZoneId,
  name: string,
...
export function getHoursWithOffset(offset: number): HourDay[] {
  ...
  const toHourDayPair = (hours: number[], dayOffset: number): HourDay[] =>
    hours.map((x) => ({ hour: x, dayOffset }));
  const hours24 = Array(24)
    .fill(1)
    .map((x, i) => i);
  const result = [-1, 0, 1]
    .map((dayOffset) => toHourDayPair(hours24, dayOffset))
